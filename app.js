...
(function() {
    ...
    const eras = {
        ...
        '1900': { label: '1900s', name: 'Sepia clásico', yearRange: [1895, 1915] },
        ...
        '1990': { label: '1990s', name: 'Cámara desechable', yearRange: [1988, 1999] }
    };
    ...
    $('btn-flash').addEventListener('click', () => {
        ...
        $('btn-flash').style.opacity = flashEnabled ? '1' : '0.5';
    });
    ...
    // Close era picker on tap outside
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.era-selector') && !e.target.closest('.era-picker')) {
            eraPicker.classList.add('hidden');
        }
    });
    ...
    function captureFromVideo() {
        ...
        showProcessing();
        setTimeout(() => {
            applyVintageFilter(previewCanvas);
        }, 100);
    }
    ...
    function applyVintageFilter(sourceCanvas) {
        ...
        const item = { thumb: thumbData, full: fullData, era: currentEra, date: dateStr, ts: Date.now() };
    ...
    function applyDaguerreotype(data, w, h) {
        ...
        // Blur edges (simulate soft focus)
    ...
    function applySilentFilm(data, w, h) {
        for (let i = 0; i < data.length; i += 4) {
            let gray = data[i] * 0.299 + data[i+1] * 0.587 + data[i+2] * 0.114;
            // High contrast B&W with slight warm tint
    ...
    function applyPolaroid70(data, w, h) {
        for (let i = 0; i < data.length; i += 4) {
            // Faded warm look
            data[i] = clamp(data[i] * 1.1 + 15);       // boost red
            data[i+1] = clamp(data[i+1] * 1.0 + 5);    // slight green
            data[i+2] = clamp(data[i+2] * 0.8);         // reduce blue
    ...
    function applyDisposable90(data, w, h) {
        for (let i = 0; i < data.length; i += 4) {
            // Slightly oversaturated, green/yellow tint
    ...
    function blurEdges(data, w, h, strength) {
        // Simple edge darkening/softening
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                const dx = (x - w/2) / (w/2);
                const dy = (y - h/2) / (h/2);
